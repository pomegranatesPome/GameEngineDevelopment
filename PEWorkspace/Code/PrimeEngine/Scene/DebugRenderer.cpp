...
void DebugRenderer::createAABBMesh(Vector3 min, Vector3 max)
{
	if (EnableDebugRendering && m_numAvailableLineLists)
	{
		Matrix4x4 m;
		m.loadIdentity();
		//Vector3 color(0.655, 0.87, 0.922);
		Vector3 color(0.45, 0.45, 0.45);
		Vector3 drawLine[4];
		drawLine[1] = color;
		drawLine[3] = color;


		// 3 edges pointing out from min 
		drawLine[0] = min;
		drawLine[2] = min;
		drawLine[2].m_x = max.getX();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		
		drawLine[2] = min;
		drawLine[2].m_y= max.getY();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);

		drawLine[2] = min;
		drawLine[2].m_z = max.getZ();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);

		// 2 edges pointing out of (maxX, minY, minZ)
		drawLine[0] = min;
		drawLine[0].m_x = max.getX();
		drawLine[2] = max;
		drawLine[2].m_z = min.getZ();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		drawLine[2] = max;
		drawLine[2].m_y = min.getY();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);

		// 2 edges pointing out from (minx, maxY, minZ)
		drawLine[0] = min;
		drawLine[0].m_y = max.getY();
		drawLine[2] = max;
		drawLine[2].m_z = min.getZ();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		drawLine[2] = max;
		drawLine[2].m_x = min.getX();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);

		// 2 edges from (minx, miny, maxZ)
		drawLine[0] = min;
		drawLine[0].m_z = max.getZ();
		drawLine[2] = max;
		drawLine[2].m_x = min.getX();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		drawLine[2] = max;
		...
		// 3 edges pointing out from max
		drawLine[2] = max;
		drawLine[0] = max;
		drawLine[0].m_x = min.getX();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		drawLine[0] = max;
		drawLine[0].m_y = min.getY();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
		drawLine[0] = max;
		drawLine[0].m_z = min.getZ();
		DebugRenderer::createLineMesh(false, m, &drawLine[0].m_x, 2, 0);
