	...
	PEStaticVector<Handle, 4> &effects, PEStaticVector<Handle, 4> &shadowMapEffects, PEStaticVector<Handle, 4> &instanceEffects)
{
	...
	if (curMatCpu.m_preferredTechName[0])
	{
		...
		// Milestone3: see if this mesh needs an outline pass.
		if (curMatCpu.m_useOutline == 1)
		{
			sprintf(preferredTechName, "%s_Outline_Tech", toString(format));
		...
		// Milestone3: Add rendering technique from preferred
	...
	if (!hasBlendShapes)
	{
		if (format == PEVertexFormat_DetailedMesh || format == PEVertexFormat_DetailedSkin)
		{
			...
			{	
				// TODO: Milestone2 doesnt support outlining for skins
...
{
	PositionBufferCPU * pBufferCPU = m_hPositionBufferCPU.getObject<PositionBufferCPU>();
	int numOfVerts = pBufferCPU->m_values.m_size / 3;
	// initialize first value
	m_AABB.m_min.m_x = pBufferCPU->m_values[0];
	m_AABB.m_max.m_x = pBufferCPU->m_values[0];
	m_AABB.m_min.m_y = pBufferCPU->m_values[1];
	m_AABB.m_max.m_y = pBufferCPU->m_values[1];
	m_AABB.m_min.m_z = pBufferCPU->m_values[2];
	m_AABB.m_max.m_z = pBufferCPU->m_values[2];

	// iterate through all verts 
	for (int i = 1; i < numOfVerts; i++)
	{
		m_AABB.m_min.m_x = (pBufferCPU->m_values[3 * i] < m_AABB.m_min.m_x) ? pBufferCPU->m_values[3 * i] : m_AABB.m_min.m_x;
		m_AABB.m_max.m_x = (pBufferCPU->m_values[3 * i] > m_AABB.m_max.m_x) ? pBufferCPU->m_values[3 * i] : m_AABB.m_max.m_x;
		m_AABB.m_min.m_y = (pBufferCPU->m_values[3 * i + 1] < m_AABB.m_min.m_y) ? pBufferCPU->m_values[3 * i + 1] : m_AABB.m_min.m_y;
		m_AABB.m_max.m_y = (pBufferCPU->m_values[3 * i + 1] > m_AABB.m_max.m_y) ? pBufferCPU->m_values[3 * i + 1] : m_AABB.m_max.m_y;
		m_AABB.m_min.m_z = (pBufferCPU->m_values[3 * i + 2] < m_AABB.m_min.m_z) ? pBufferCPU->m_values[3 * i + 2] : m_AABB.m_min.m_z;
		m_AABB.m_max.m_z = (pBufferCPU->m_values[3 * i + 2] > m_AABB.m_max.m_z) ? pBufferCPU->m_values[3 * i + 2] : m_AABB.m_max.m_z;
	}

	PEINFO("AABB calculated: %f, %f, %f, %f, %f, %f. \n", m_AABB.m_min.m_x, m_AABB.m_max.m_x, m_AABB.m_min.m_y, m_AABB.m_max.m_y, m_AABB.m_min.m_z, m_AABB.m_max.m_z);
