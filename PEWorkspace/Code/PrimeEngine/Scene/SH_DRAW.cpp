...
// Assignment 3: event handler where mesh instance is potentially added to draw list that will be submitted to graphics pipeline
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (false && pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		//get camera scene node to get frustum info
		CameraSceneNode* myCameraSN = CameraManager::Instance()->getActiveCamera()->getCamSceneNode();
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			/*if (iInst % 2)
			{
				...
				Vector3 min(0.2, 0.2, 0.2);
				Vector3 max(0.7, 0.7, 0.7);

				DebugRenderer::Instance()->createAABBMesh(min, max);

			}*/
			PE::Handle hmeshSceneNode = pInst->getFirstParentByType<SceneNode>();

			// get this mesh instance's transform to calculate its min and max corner in world space
			Matrix4x4 mmeshTransform;
			if (hmeshSceneNode.isValid()) { // get transformation 
				mmeshTransform = hmeshSceneNode.getObject<SceneNode>()->m_worldTransform;
			}
			else {
				OutputDebugStringA("ERROR: hmeshSceneNode is not valid\n");
			}

			Vector3 worldMin = mmeshTransform * pMeshCaller->m_AABB.m_min;
			Vector3 worldMax = mmeshTransform * pMeshCaller->m_AABB.m_max;

			if (myCameraSN->viewFrustumMesh_intersection_check(worldMin, worldMax)) // Returns true means that it is not culled
			{
				pInst->m_culledOut = false;
				++pMeshCaller->m_numVisibleInstances;
				DebugRenderer::Instance()->createAABBMesh(worldMin, worldMax);
			}
			else
			{
				pInst->m_culledOut = true;
			}
			// To DISABLE THE CULLING, remove false &&. 
			if (pInst->m_culledOut == true) {
				pInst->m_culledOut = false;
				++pMeshCaller->m_numVisibleInstances;
				DebugRenderer::Instance()->createAABBMesh(worldMin, worldMax);
			}
		...
		Events::Event_GATHER_DRAWCALLS *pDrawEvent, Events::Event_GATHER_DRAWCALLS_Z_ONLY *pZOnlyDrawEvent)
{
	...
	if (pDrawEvent)
	{
		...
		if (pMeshCaller->m_numVisibleInstances > 1)
		{
			...
			if (haveInstancesAndInstanceEffect) // not executed in Milestone2
			{
	...
	for (PrimitiveTypes::UInt32 iEffect = 0; iEffect < pEffectsForRange->m_size; ++iEffect)
	{
		...
		for (int iRenderGroup = 0; iRenderGroup < numRenderGroups; ++iRenderGroup)
		{
			...
			if (haveInstancesAndInstanceEffect) // Milestone 2 not executed
			{
			...
			for (PrimitiveTypes::UInt32 _iBoneSegment = 0; _iBoneSegment < numJointSegments; _iBoneSegment++)
			{
                ...
                {	
					// MILESTONE 2: THIS CLAUSE IS EXECUTED
				...
				if (haveInstancesAndInstanceEffect) // instanced version, not executed in Milestone2 
				{
					if (hasJointSegments)
					{
