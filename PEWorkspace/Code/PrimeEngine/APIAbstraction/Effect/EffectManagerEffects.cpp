...
#	if false && (APIABSTRACTION_D3D9 || APIABSTRACTION_D3D11)
...
#	endif
	...
	}
	{
		...
		pEffect->m_psInputFamily = EffectPSInputFamily::STD_MESH_SHADOWED_PS_IN;
	...
	// Milestone3 reorganize effects and tech names
	{
		Handle hEffect("EFFECT", sizeof(Effect));
		Effect *pEffect = new(hEffect) Effect(*m_pContext, m_arena, hEffect);
		pEffect->loadTechnique(
		...
		m_map.add("StdMesh_Cel_Tech", hEffect);
	}
...

	{
		...
		Effect *pEffect = new(hEffect) Effect(*m_pContext, m_arena, hEffect);
		pEffect->loadTechnique(
			"DetailedMesh_Cel_VS", "main",
			NULL, NULL, // geometry shader
			"DetailedMesh_Cel_PS", "main",
			NULL, NULL, // compute shader
			PERasterizerState_SolidTriBackCull,
			PEDepthStencilState_ZBuffer, PEAlphaBlendState_NoBlend, // depth stencil, blend states
			"DetailedMesh_Cel_Tech");

		pEffect->m_psInputFamily = EffectPSInputFamily::DETAILED_MESH_CEL_PS_IN;
		pEffect->m_effectDrawOrder = EffectDrawOrder::First;
		m_map.add("DetailedMesh_Cel_Tech", hEffect);
	}
	...
	// MILESTONE2 STD MESH OUTLINING PASS
	{
		...
		pEffect->loadTechnique(
			...
			"Outline_PS", "main",
			NULL, NULL, // compute shader
			PERasterizerState_SolidTriFrontCull,
			PEDepthStencilState_ZBuffer, PEAlphaBlendState_NoBlend, // depth stencil, blend states
			"StdMesh_Outline_Tech");
		pEffect->m_psInputFamily = EffectPSInputFamily::STD_MESH_PS_IN;
		pEffect->m_effectDrawOrder = EffectDrawOrder::First;
		m_map.add("StdMesh_Outline_Tech", hEffect);
	}

	// MILESTONE2 DETAILED MESH OUTLINING PASS
	{

		Handle hEffect("EFFECT", sizeof(Effect));
		Effect *pEffect = new(hEffect) Effect(*m_pContext, m_arena, hEffect);
		pEffect->loadTechnique(
			"Outline_DetailedMesh_VS", "main",
			NULL, NULL, // geometry shader
			"Outline_DetailedMesh_PS", "main",
			NULL, NULL, // compute shader
			PERasterizerState_SolidTriFrontCull,
			PEDepthStencilState_ZBuffer, PEAlphaBlendState_NoBlend, // depth stencil, blend states
			"DetailedMesh_Outline_Tech");
		pEffect->m_psInputFamily = EffectPSInputFamily::DETAILED_MESH_SHADOWED_PS_IN;
		pEffect->m_effectDrawOrder = EffectDrawOrder::First;
		m_map.add("DetailedMesh_Outline_Tech", hEffect);
	}
