...
struct DETAILED_MESH_INSTANCED_VS_IN
{
	...
	#define VS_wrapper_DETAILED_MESH_SHADOWED(func) \
     ...
     // Milestone3 - M2 cleanup
    #define VS_wrapper_DETAILED_MESH_CEL(func) \
		DETAILED_MESH_CEL_PS_IN main(DETAILED_MESH_VS_IN API_NOSTRIP vIn) { \
			DETAILED_MESH_CEL_PS_IN pIn; \
			pIn = func(vIn); \
			return pIn; \
    }

     // Milestone 2
    #define VS_wrapper_OUTLINE_DETAILED_MESH(func) \
		DETAILED_MESH_SHADOWED_PS_IN main(DETAILED_MESH_VS_IN API_NOSTRIP vIn) { \
			DETAILED_MESH_SHADOWED_PS_IN pIn; \
			pIn = func(vIn); \
			return pIn; \
...
struct DETAILED_MESH_CEL_PS_IN
{ // Milestone 1
    float4 iPosH     API_SEMANTIC(PIPELINE_POSITION);
    float3 iPosW     API_SEMANTIC(PSIN_EXTRA_POSITION);
    float2 iTexCoord API_SEMANTIC(TEXCOORD0);
...
struct OUTLINE_DETAILED_MESH_SHADOWED_PS_IN
{
    // Milestone2
    float4 iPosH     API_SEMANTIC(PIPELINE_POSITION);
    float3 iPosW     API_SEMANTIC(PSIN_EXTRA_POSITION);
    ...
    #define PS_wrapper_DETAILED_MESH_SHADOWED(func) \
     ...
     // Milestone3 - M2 cleanup
	#define PS_wrapper_DETAILED_MESH_CEL(func) \
        float4 main(DETAILED_MESH_CEL_PS_IN pIn) : PSOUT_COLOR \
        { \
            return func(pIn); \
        }
    // Milestone 2
    #define PS_wrapper_OUTLINE_DETAILED_MESH(func) \
        float4 main(DETAILED_MESH_SHADOWED_PS_IN pIn) : PSOUT_COLOR \
        { \
            return func(pIn); \
        }
